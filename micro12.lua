...
-- Chong da tu bam chon macro trong danh sach chua. Neu roi thi "Tu chon ai"
-- KHONG duoc doi macro nua (loi da gap: chon macro xong vao van moi no tu nhay
-- sang macro khac vi thay macro do khop ten ai).
local chonTay      = false
...
function M.tuChonMacro()
    if chonTay then return true end        -- chong da chon tay -> khong doi
    local ai = thongTinAi()
    ...
    chonTay = false                    -- day la TU chon, khong phai chon tay
...
function M.thuTuChay()
    ...
    -- "Van da bat dau" tinh theo BA dau hieu, khong chi `start`:
    --   * workspace.start / dungeonStarted  (PlaceManager.HasDungeonStarted,
    --     PlaceManager.lua:705-730)
    --   * dungeonProgress == "inProgress"   (readyButton tu huy luc do,
    --     ui/readyButton/LocalScript.lua:6-9)
    -- Thieu dau hieu thu ba la acc KHONG PHAI CHU PHONG co the doi mai: no
    -- khong co startButton nen `start` ben nay khong bao gio thay doi.
    local batDau = (st ~= nil and st.Value == true)
                or (ds ~= nil and ds.Value == true)
                or (tt == "inProgress")
    local xong   = daXongAi()
    ...
    -- MOI NHANH DUNG DEU PHAI NOI RO LY DO. Bo nay truoc day `return` im lang
    -- nen chong bam "Playback: BAT" ma khong chay thi khong biet tac o dau.
    if M.dangGhi then M.lyDoCho = "dang GHI macro" return end
    if M.dangChay then M.lyDoCho = nil return end
    if not tenDangChon then
        if M.autoChonAi then pcall(M.tuChonMacro) end
        if not tenDangChon then
            M.lyDoCho = "CHUA CHON MACRO - bam ten macro trong danh sach"
            return
        end
    end
    if #macro.khung == 0 then
        M.lyDoCho = "macro [" .. tostring(tenDangChon) .. "] khong co khung nao"
        return
    end
    if xong then M.lyDoCho = "ai da xong - cho van moi" return end
    if not batDau then
        M.lyDoCho = "van chua bat dau (start/dungeonStarted/inProgress deu tat)"
        return
    end
    if daChayVanNay then M.lyDoCho = "da chay macro van nay roi" return end
    if M.mocChay and os.clock() < M.mocChay then
        M.lyDoCho = ("cho %ds sau start"):format(math.ceil(M.mocChay - os.clock()))
        return
    end
...
function M.napTen(ten)
    ...
    ganKhungChoSuKien()
    tenDangChon = ten
    -- Nap mot macro CO TEN nghia la co nguoi chon no (bam GUI, hoac doc tu
    -- config da luu) -> khoa lai, "Tu chon ai" khong duoc doi nua.
    -- Rieng tuChonMacro() tu no ha co nay xuong sau khi goi.
    chonTay = true
    return true
end

-- Cho ben ngoai (debug / test) doc duoc dang chon macro nao.
function M.macroDaChon()
    return tenDangChon, #macro.khung, #macro.sukien
end
...
--== LUU / NAP CONFIG ==========================================================
-- Chong yeu cau: chon macro -> bam Playback -> bam LUU CONFIG; tu lan sau chay
-- script la no doc lai tu workspace va tu bat y nhu the.
-- Luu chung MOT file (khong theo ten acc) de con gui ca bo cho nguoi khac dung.
local TEP_CFG = "macro_ai_cfg.json"

function M.luuCfg()
    if type(writefile) ~= "function" then return false, "executor khong co writefile" end
    if not tenDangChon then return false, "chua chon macro nao" end
    if #macro.khung == 0 then return false, "macro dang chon khong co khung nao" end
    local ok = pcall(function()
        writefile(TEP_CFG, HttpService:JSONEncode({
            macro       = tenDangChon,
            autoPlay    = M.autoPlay,
            autoStart   = M.autoStart,
            autoReplay  = M.autoReplay,
            autoChonAi  = M.autoChonAi,
            choSauStart = CFG.CHO_SAU_START,
            dongBoTocDo = CFG.DONG_BO_TOC_DO,
        }))
    end)
    if not ok then return false, "ghi file loi" end
    M.daNapCfg = true
...
-- Tra ve: co nap duoc khong, va loi neu macro trong config khong con file.
function M.napCfg()
    if type(isfile) ~= "function" or type(readfile) ~= "function" then return false end
    if not isfile(TEP_CFG) then return false end
    local ok, c = pcall(function() return HttpService:JSONDecode(readfile(TEP_CFG)) end)
    if not ok or type(c) ~= "table" then return false, "file config hong" end
    if type(c.autoStart) == "boolean" then M.autoStart = c.autoStart end
    if type(c.autoReplay) == "boolean" then M.autoReplay = c.autoReplay end
    if type(c.autoChonAi) == "boolean" then M.autoChonAi = c.autoChonAi end
    if type(c.dongBoTocDo) == "boolean" then CFG.DONG_BO_TOC_DO = c.dongBoTocDo end
    if type(c.choSauStart) == "number" then
        CFG.CHO_SAU_START = math.max(0, math.min(60, c.choSauStart))
    end
    local loi = nil
    if type(c.macro) == "string" and c.macro ~= "" then
        local okNap, vi = M.napTen(c.macro)   -- napTen tu khoa chonTay
        if not okNap then
            loi = ("macro [%s] trong config khong nap duoc: %s"):format(
                c.macro, tostring(vi))
        end
    end
    -- Bat Playback SAU khi da nap macro, va dat moc cho de khong chay ngay luc
    -- nhan vat / tool con dang tai.
    if type(c.autoPlay) == "boolean" then
        M.autoPlay = c.autoPlay
        if M.autoPlay then M.mocChay = os.clock() + CFG.CHO_SAU_START end
    end
    M.daNapCfg = true
    return true, loi
end
...
function M.xoaCfg()
    if type(delfile) == "function" then pcall(function() delfile(TEP_CFG) end) end
    M.daNapCfg = false
    return true
end
...
bang.Size = UDim2.new(0, 230, 0, 534)
-- Mac dinh nam BEN PHAI man hinh (chong yeu cau). Van keo tha duoc.
bang.Position = UDim2.new(1, -250, 0.5, -267)
...
function M.capNhatGui()
    ...
    if M.daNapCfg then d3 = d3 .. "  | config DA LUU" end
    -- Hien RO vi sao Playback chua chay (truoc day im lang nen khong ai biet).
    if M.autoPlay and not M.dangChay and not M.dangGhi and M.lyDoCho then
        d3 = d3 .. XUONG_DONG .. "Playback: " .. M.lyDoCho
    end
...
UI.luu = taoNut("LUU CONFIG", 8, 444, 104, Color3.fromRGB(50, 110, 100), function()
    local ok, vi = M.luuCfg()
    trangThai.Text = ok
        and ("da LUU CONFIG: macro [%s], Playback=%s, cho %ds"):format(
            tostring(tenDangChon), M.autoPlay and "BAT" or "TAT", CFG.CHO_SAU_START)
        or ("luu that bai: " .. tostring(vi))
end)

UI.xoaCfg = taoNut("XOA CONFIG", 118, 444, 104, Color3.fromRGB(110, 50, 50), function()
    M.xoaCfg()
    trangThai.Text = "da XOA config - lan sau mo len se khong tu bat"
end)

local ghiChu = Instance.new("TextLabel")
ghiChu.Size = UDim2.new(1, -16, 0, 54)
ghiChu.Position = UDim2.new(0, 8, 0, 474)
...
nutThu.MouseButton1Click:Connect(function()
    ...
    bang.Size = thu and UDim2.new(0, 230, 0, 30) or UDim2.new(0, 230, 0, 534)
...
M.napDanhSach()
local okCfg, loiCfg = M.napCfg()
if loiCfg then warn("[MACRO AI] " .. tostring(loiCfg)) end
if M.autoChonAi then pcall(M.tuChonMacro) end
M.veDanhSach()
M.capNhatGui()
if loiCfg then trangThai.Text = tostring(loiCfg) end
print(("[MACRO AI v2] da bat, %d macro da luu. config=%s macro=%s Playback=%s cho=%ds")
    :format(#danhSach, okCfg and "DA NAP" or "chua co", tostring(tenDangChon),
        M.autoPlay and "BAT" or "TAT", CFG.CHO_SAU_START))
print("[MACRO AI v2] Tat bang:  _G.__MACRO_AI__.tat()")

...
-- BAN SKILL. Doc ky LocalScript that cua tung chieu
-- (ReplicatedStorage/abilities/<ten>/LocalScript.lua):
--
--     script_Parent:WaitForChild("localEvent").Event:Connect(function()
--         local busyCasting = Character:FindFirstChild("busyCasting")
--         local v2 = (busyCasting == nil) and false or (busyCasting.Value == false)
--         if not (script_Parent.cooldown.Value <= 0 and (u1 and v2)) then
--             Humanoid:UnequipTools()
--             return                          -- IM LANG BO, khong ban gi ca
--         end
--         u1 = false
--         task.delay(cooldownLength, function() u1 = true end)
--         script_Parent.spellEvent:FireServer()      -- CU BAN THAT
--         animationCache.load(Humanoid, spellAnim):Play()
--     end)
--
-- Rut ra:
--   * `localEvent` chi la CO SUNG. `spellEvent` (RemoteEvent nam TRONG tool)
--     moi la dan. Chong dua dump tu dien thoai dung cai nay:
--         Backpack["Agony Orbs"].spellEvent:FireServer()
--   * Co BA cua chan truoc khi no ban: `cooldown`, debounce noi bo `u1`, va
--     `busyCasting`. Thieu mot cai la no UnequipTools roi thoat - khong ban,
--     khong bao loi. Rieng `busyCasting == nil` thi CHAN LUON.
--   * Bot khong doc duoc `u1` (bien noi bo cua LocalScript).
--
-- Nen: uu tien duong chinh thuc `localEvent` (co animation, dung debounce cua
-- game). Chi khi BIET CHAC no se bi chan thi ban THANG `spellEvent` - dung y
-- dump cua chong. Khong bao gio goi ca hai, keo ban hai lan.
local function banSkill(slot)
    ...
    local ch = char()
    local busy = ch and ch:FindFirstChild("busyCasting")
    -- Dung dieu kien cua LocalScript: chi no chiu chay khi busyCasting ton tai
    -- VA dang false.
    local coSungChay = (busy ~= nil) and (busy.Value == false)

    local ev    = tool:FindFirstChild("localEvent")
    local spell = tool:FindFirstChild("spellEvent")

    if coSungChay and ev and ev:IsA("BindableEvent") then
        if not pcall(function() ev:Fire() end) then return false end
    elseif spell and spell:IsA("RemoteEvent") then
        -- LocalScript chac chan bi chan -> ban thang dan.
...
-- AUTO PLAYBACK: moi van moi thi tu chay macro dang chon.
--
-- LOI DA SUA (chong: "cai playback no ko auto bat lai kia"):
-- Ban dau em ha co `daChayVanNay` theo TRANG THAI:
--     if daXong or not daBatDau then daChayVanNay = false end
-- Nhung khi choi lai bang `replayDungeon`, `Workspace.start` co the GIU
-- NGUYEN true va `dungeonFinished` chi nhay ve false - khong co luc nao
-- ca hai cung "tat" de co duoc ha xuong => van sau khong bao gio chay lai.
--
-- Nay bat theo CANH DOI (doi tu gia tri nay sang gia tri kia), khong theo
-- trang thai dung yen:
--   * start:           false -> true   = van moi bat dau
--   * dungeonFinished: true  -> false  = da sang van moi
--   * dungeonName doi           = sang ai khac
-- Bat cu canh nao trong ba cai tren xay ra deu mo khoa cho van moi.
local startTruoc, xongTruoc, tenAiTruoc = nil, nil, nil

function M.thuTuChay()
    local st  = workspace:FindFirstChild("start")
    local dn  = workspace:FindFirstChild("dungeonName")
    local daBatDau = st ~= nil and st.Value == true
    local daXong   = daXongAi()
    local tenAi    = dn and dn.Value or nil

    -- CANH DOI -> mo khoa cho van moi
    if (startTruoc == false and daBatDau == true)
        or (xongTruoc == true and daXong == false)
        or (tenAiTruoc ~= nil and tenAi ~= nil and tenAi ~= tenAiTruoc) then
        daChayVanNay = false
    end
    startTruoc, xongTruoc, tenAiTruoc = daBatDau, daXong, tenAi

    if M.dangGhi or M.dangChay then return end
    if daXong or not daBatDau then return end   -- chua vao tran / da xong
    if daChayVanNay then return end
    if not tenDangChon then return end          -- chua chon macro nao
...
bang.Size = UDim2.new(0, 230, 0, 524)
bang.Position = UDim2.new(0, 20, 0.5, -262)
...
local nutGhi, nutChay, nutAutoStart, nutAutoReplay, nutSwap
local nutQ, nutE, nutAutoPlay
...
function M.capNhatGui()
    ...
    -- Cho chong nhin thay dang o bo nao: ten tool dang giu slot q va e.
    if nutSwap then
        local tq, te = tenToolO("q"), tenToolO("e")
        if tq or te then
            nutSwap.Text = ("SWAP  q:%s  e:%s"):format(
                tq and string.sub(tq, 1, 9) or "?",
                te and string.sub(te, 1, 9) or "?")
        else
            nutSwap.Text = "SWAP (doi bo skill)"
        end
    end
    -- Ten hai nut chieu doc lai tu Backpack moi lan -> Swap xong tu doi ten.
    if nutQ then
        local t = tenToolO("q")
        nutQ.Text = t and ("Q: " .. string.sub(t, 1, 12)) or "Q: (khong co)"
    end
    if nutE then
        local t = tenToolO("e")
        nutE.Text = t and ("E: " .. string.sub(t, 1, 12)) or "E: (khong co)"
    end
    if nutAutoPlay then
        nutAutoPlay.Text = "Auto Playback: " .. (M.autoPlay and "BAT" or "TAT")
        nutAutoPlay.BackgroundColor3 = M.autoPlay
...
-- HAI NUT XAI CHIEU RIENG (chong yeu cau: "detect toi co chieu gi thi doi ten
-- nut do", "co 2 skill thi tach rieng ra 2 nut", "khi swap nho fetch lai ten").
-- Ten nut duoc doc TU BACKPACK moi lan capNhatGui chay (1 giay mot lan) nen
-- Swap xong la ten tu doi theo, khong can lam gi them.
...
local function bamChieu(slot)
    ...
    if ok and M.dangGhi then ghiSuKien(slot) end
    if ok then
        trangThai.Text = M.dangGhi
            and ("da xai [" .. ten .. "] + ghi vao macro")
            or ("da xai [" .. ten .. "]")
    else
        -- banSkill tra false khi: chua co tool, cooldown > 0, hoac ca localEvent
        -- lan spellEvent deu khong dung duoc.
        local tool = toolSkill(slot)
        local cd = tool and tool:FindFirstChild("cooldown")
...
nutQ = taoNut("Q", 8, 296, 104, Color3.fromRGB(45, 95, 130), function()
    bamChieu("q")
end)

nutE = taoNut("E", 118, 296, 104, Color3.fromRGB(45, 95, 130), function()
    bamChieu("e")
end)

-- NUT SWAP RIENG (chong yeu cau): bam vao day la GOI THANG remote
--     ReplicatedStorage.remotes.swapAbilitySet:FireServer()
-- va neu dang ghi thi GHI LUON vao macro. Nho vay khong con phu thuoc viec bat
-- duoc nut Swap that cua game (tren dien thoai thu do rat kho bat).
nutSwap = taoNut("SWAP (doi bo skill)", 8, 326, 214,
    Color3.fromRGB(120, 80, 165), function()
    local ok = banSwap()
    if ok and M.dangGhi then ghiSuKien("swap") end
    if ok then
        trangThai.Text = M.dangGhi
            and "da SWAP + ghi vao macro"
            or "da SWAP"
    else
        trangThai.Text = "swap chua duoc, cho 0.5 giay roi bam lai"
    end
end)

nutAutoStart = taoNut("Auto Start: BAT", 8, 356, 214, Color3.fromRGB(40, 110, 70), function()
    M.autoStart = not M.autoStart
    M.capNhatGui()
end)

nutAutoReplay = taoNut("Auto Replay: BAT", 8, 386, 214, Color3.fromRGB(40, 110, 70), function()
    M.autoReplay = not M.autoReplay
    M.capNhatGui()
end)

-- AUTO PLAYBACK (chong yeu cau): chon macro nao thi MOI LAN VAO AI MOI no tu
-- chay macro do. Moc nhan biet "van moi da bat dau" la Workspace.start == true
-- (chinh cai ma Auto Start bam vao), va van chua ket thuc.
nutAutoPlay = taoNut("Auto Playback: TAT", 8, 416, 214,
    Color3.fromRGB(70, 50, 50), function()
    M.autoPlay = not M.autoPlay
    if not M.autoPlay then daChayVanNay = false end
    M.capNhatGui()
end)

local ghiChu = Instance.new("TextLabel")
ghiChu.Size = UDim2.new(1, -16, 0, 62)
ghiChu.Position = UDim2.new(0, 8, 0, 448)
ghiChu.BackgroundTransparency = 1
ghiChu.Text = "Nut Q/E tu doi ten theo chieu dang mang." .. XUONG_DONG .. "Bam Q/E/SWAP o day = xai that + ghi vao macro." .. XUONG_DONG .. "Auto Playback: moi van moi tu chay macro dang chon."

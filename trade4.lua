--==============================================================================
--  ALIMIDAI AUTO TRADE  ·  v2.2.1  ·  MỘT file duy nhất
--
...
--
--  ---------------------------------------------------------------------------
--  MẶC ĐỊNH: tự kết nối main.py/alimidai.exe tại http://127.0.0.1:3000.
--  Có thể chạy trực tiếp file này trong autoexec hoặc dùng loader.lua.
--  MainAcc và Items lấy từ tool; chưa kết nối tool thì chưa trade.
--
--  DÙNG ĐỘC LẬP (phải bật Standalone = true rõ ràng):
--
--      getgenv().AlimidaiConfig = {
--          Standalone = true,              -- bỏ qua tool, không báo done về tool
...
--  Để trong autoexec cũng được: client nào đang do launcher điều khiển thì
--  bản loadstring tự nhường, không chạy song song.
--
--  ---------------------------------------------------------------------------
--  DÙNG VỚI LAUNCHER (main.py):
--
--  loader.lua trong autoexec lấy chunk từ /bootstrap của main.py qua localhost.
--  Tool chèn SETTINGS_JSON cho đúng username + JobId + lượt chạy hiện tại.
--  Có biến đó -> chạy "chế độ coordinator" (báo HTTP về launcher).
--  Không có -> tự lấy lượt chạy từ localhost, KHÔNG tự chạy độc lập.
--==============================================================================

local TR_VERSION = "2.2.1"
...
--
-- owner = "standalone" (loadstring/autoexec) hoặc "coordinator" (main.py).
-- Engine của launcher nằm ở khoá getgenv RIÊNG, nên bản loadstring (kể cả bản
-- cũ trên GitHub) không tìm thấy để tắt nhầm nó.
--==============================================================================
local STANDALONE_KEYS  = { "AlimidaiTrade", "AnimeDiceTrade" }
local COORDINATOR_KEY  = "AlimidaiCoordinatorTrade"
local ALL_KEYS         = { "AlimidaiCoordinatorTrade", "AlimidaiTrade", "AnimeDiceTrade" }

local function coordinated()
	return env.AlimidaiCoordinated ~= nil
end

local function startTrade(overrides, owner)
	owner = owner == "coordinator" and "coordinator" or "standalone"
	...
	if owner == "standalone" and coordinated() then
		warn("[ALIMIDAI] Launcher đang điều khiển client này -> bản loadstring/autoexec tự nhường.")
		return nil
	end

	-- Tắt bản đang chạy trước khi dựng bản mới. Bản loadstring chỉ được tắt
	-- bản loadstring; launcher thì tắt hết.
	for _, key in ipairs(owner == "coordinator" and ALL_KEYS or STANDALONE_KEYS) do
		local old = env[key]
	...
	-- CHẠY
	--==========================================================================
	-- Lúc chờ remote ở trên có thể launcher đã nhận client này. Kiểm tra lại
	-- rồi đăng ký NGAY (không có lệnh chờ nào ở giữa), để không bao giờ có 2
	-- engine cùng chạy trong 1 client.
	if owner == "standalone" and coordinated() then
		warn("[ALIMIDAI] Launcher đã nhận client này trong lúc khởi động -> bản loadstring tự nhường.")
		return nil
	end
	...
	handle = {
		...
		Stop = function()
			...
			for _, key in ipairs(ALL_KEYS) do
				if env[key] == handle then env[key] = nil end
			end
			print("[TRADE] đã dừng.")
		end,
	}
	if owner == "coordinator" then
		env[COORDINATOR_KEY] = handle
	else
		for _, key in ipairs(STANDALONE_KEYS) do env[key] = handle end
	end

	RT.role = TR.role(LP.Name, CFG.MainAcc)
	UI.build()
	if not RT.running then
		-- Bị tắt trong lúc dựng giao diện.
		if UI.gui then pcall(function() UI.gui:Destroy() end) end
		return handle
	end
...
local function runStandalone()
	...
	return startTrade(nil, "standalone")
end
...
local function runCoordinator(settings)
	...
	-- Nhận quyền điều khiển client TRƯỚC mọi lệnh chờ: bản loadstring/autoexec
	-- khởi động sau lúc này sẽ tự nhường.
	env.AlimidaiCoordinated = identity.attempt_id
	for _, key in ipairs(ALL_KEYS) do
		local old = env[key]
	...
	local engine   -- engine trade của launcher; chỉ launcher giữ tham chiếu này

	-- Bản loadstring cũ (không biết nhường) có thể vẫn khởi động được song song.
	-- Thấy là tắt ngay, để client chỉ có 1 engine trade.
	local function evictOthers()
		for _, key in ipairs(STANDALONE_KEYS) do
			local other = env[key]
			if type(other) == "table" and other ~= engine then
				if type(other.Stop) == "function" then pcall(other.Stop) end
				env[key] = nil
				warn("[COORDINATOR] Tắt engine trade chạy song song (autoexec/loadstring).")
			end
		end
	end
	...
	local function run()
		...
		task.spawn(function()
			while active() do
				...
				if status == 409 or (result and result.stop) then
					stopped = true
					if engine then pcall(engine.Stop) end
				end
			end
		end)
		...
		local trade = assert(engine, "Trade module did not initialize")
		...
		while active() do
			evictOthers()
			if not trade.RT.running then
				error("Engine trade bị tắt ngoài ý muốn (có script khác chạy cùng?)")
			end
			...
			-- Launcher kiểm tra acc farm dần dần, nên danh sách được nhận có thể dài thêm.
			if response and settings.role == "main" and type(response.allowed) == "table" then
				trade.CFG.OnlyFrom = response.allowed
			end
	...
	if env.AlimidaiCoordinated == identity.attempt_id then env.AlimidaiCoordinated = nil end
end

--==============================================================================
-- VÀO ĐÂY
--==============================================================================
local function connectToTool()
	if env.AlimidaiBootstrapRunning or env.AlimidaiCoordinated then return end
	env.AlimidaiBootstrapRunning = true
	local function connect()
		if not game:IsLoaded() then game.Loaded:Wait() end
		while not Players.LocalPlayer do task.wait(0.2) end
		-- Stop a previous standalone copy before it can trade outside the ledger.
		for _, key in ipairs(STANDALONE_KEYS) do
			local old = env[key]
			if type(old) == "table" and type(old.Stop) == "function" then pcall(old.Stop) end
			env[key] = nil
		end
		local cfg = type(env.AlimidaiConfig) == "table" and env.AlimidaiConfig or {}
		local api = cfg.API or "http://127.0.0.1:3000"
		local send = request or http_request or (syn and syn.request) or (http and http.request)
		assert(type(send) == "function", "Executor needs HTTP request access to localhost")
		local lastNotice = -30
		while true do
			local ok, response = pcall(send, {
				Url = api .. "/bootstrap", Method = "POST",
				Headers = {["Content-Type"] = "application/json"},
				Body = Http:JSONEncode({username=Players.LocalPlayer.Name,
					place_id=tostring(game.PlaceId), job_id=game.JobId}),
			})
			local message = "Cannot connect to tool"
			if ok and response then
				local decoded, payload = pcall(Http.JSONDecode, Http, response.Body)
				if response.StatusCode == 200 and decoded and type(payload) == "table"
					and payload.status == "ok" and type(payload.source) == "string"
					and type(payload.attempt_id) == "string" then
					if env.AlimidaiAttempt == payload.attempt_id then return end
					local chunk, compileError = loadstring(payload.source, "@alimidai_assigned")
					assert(chunk, compileError)
					chunk()
					return
				end
				message = "HTTP " .. tostring(response.StatusCode)
				if decoded and type(payload) == "table" and payload.message then
					message = message .. " / " .. tostring(payload.message)
				end
			end
			if os.clock() - lastNotice >= 15 then
				warn("[ALIMIDAI v" .. TR_VERSION .. "] Waiting for " .. api .. ": " .. message)
				lastNotice = os.clock()
			end
			task.wait(2)
		end
	end
	local ok, failure = pcall(connect)
	env.AlimidaiBootstrapRunning = nil
	if not ok then warn("[ALIMIDAI CONNECT] " .. tostring(failure)) end
end

if INJECTED then
	runCoordinator(Http:JSONDecode(INJECTED))
elseif type(env.AlimidaiConfig) == "table" and env.AlimidaiConfig.Standalone == true then
	runStandalone()
else
	connectToTool()
end

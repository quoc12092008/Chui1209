...
-- Doi bo skill. Dqr.lua:156 ghi ro remote nay TON TAI (`swapAbilitySet`) nhung
-- bot co y khong dung. Macro thi khac: chong bam Swap that thi phai phat lai
-- dung nhu vay. Tham so duoc GHI LAI luc bam chu khong doan.
local function banSwap(thamSo)
    local r = remote("swapAbilitySet")
    if not r then return false end
    return (pcall(function()
        if thamSo and #thamSo > 0 then
            r:FireServer(unpack(thamSo))
        else
            r:FireServer()
        end
    end))
...
local function ghiSuKien(loai, args)
    ...
    macro.sukien[#macro.sukien + 1] = { t = bayGio, loai = loai, args = args }
end
...
-- LOP 1 (tot nhat, chay ca PC lan DIEN THOAI): bat dung luc game goi FireServer.
-- Bam bang phim, bang chuot hay cham nut tren man hinh deu ra cung mot lenh nay.
local function batDauHook()
    if hookXong then return true end
    ...
    local ok = pcall(function()
        ...
        cu = hookmetamethod(game, "__namecall", function(self, ...)
            if M.dangGhi and not M.dangChay then
                pcall(function(...)
                    if getnamecallmethod() ~= "FireServer" then return end
                    local ten = self.Name
                    if ten == "abilityUsed" then
                        local slot = select(1, ...)
                        if slot == "q" or slot == "e" or slot == "f" then
                            ghiSuKien(slot)
                        end
                    elseif ten == "swapAbilitySet" then
                        ghiSuKien("swap", locThamSo(...))
                    else
                        -- remote cua vu khi nam TRONG Accessory co con "Weapon"
                        local cha = self.Parent
                        if cha and cha:IsA("Accessory") and cha:FindFirstChild("Weapon") then
                            ghiSuKien("danh")
                        end
                    end
                end, ...)
...
function M.batGhi()
    ...
    if batDauHook() then
        CACH_GHI = "hook"       -- bat tan goc, chay ca PC lan dien thoai
    else
        CACH_GHI = "cooldown"   -- du phong: van chay duoc tren dien thoai
        batDauTheoCooldown()
    end
...
ghiChu.Text = "Ghi: di chuyen + danh tay, Q, E, F, Swap, Space.
Chay duoc ca PC lan dien thoai (bam nut tren man hinh van ghi).
Bam ten trong danh sach de chon macro."

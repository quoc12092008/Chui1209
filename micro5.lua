...
-- Khai bao truoc `banSkill` vi Lua chi thay local da khai bao PHIA TREN.
-- (Khong co dong nay thi `dangChoDoiBo` trong banSkill la nil -> khong chan
--  duoc gi ca, ma cung khong bao loi -> bug im lang.)
local dangChoDoiBo

local function banSkill(slot)
    -- Vua Swap xong ma ban ngay thi trung tool CU. Cho server thay tool da.
    if dangChoDoiBo and dangChoDoiBo() then return false end
...
-- DOI BO SKILL (Swap). DA DOC SOURCE, khong con doan:
--   * Ui/inventory.lua:1395  phim F:
--       elseif p126.KeyCode == Enum.KeyCode.F and not u15._swapDebounce then
--           task.defer(function() remotes.swapAbilitySet:FireServer() end)
--           task.delay(0.5, function() u15._swapDebounce = false end)
--   * Ui/abilities.lua:73    nut GUI ten "Swap":
--       abilities.Swap.MouseButton1Click -> remotes.swapAbilitySet:FireServer()
--   * Ui/abilities.lua:317   SERVER BAO VE khi doi xong:
--       remotes.abilitySetSwapped.OnClientEvent:Connect(function(p24, p25)
--           u1 = p24   -- so bo dang dung: 1 hoac 2
--
-- Chong hoi dung cho: "swap xong no khong sai skill". Co that, hai ly do:
--   1. Ban Swap xong la ban q/e NGAY. Server chua kip thay tool vao slot nen
--      luc do `toolSkill("q")` van tra TOOL CU (dang hoi chieu) -> khong ban
--      duoc. Nay: sau khi Swap phai CHO `abilitySetSwapped` bao ve.
--   2. Macro chi ghi "co bam Swap", khong ghi DANG O BO NAO. Lech mot nhip
--      Swap la tu do ve sau ban nham bo: dang o bo 2 ma macro tuong bo 1.
--      Nay moi su kien q/e deu ghi kem SO BO luc record; chay lai neu dang o
--      bo khac thi TU Swap cho khop roi moi ban.
local lanSwap  = 0
local boHienTai = 1        -- so bo dang dung, cap nhat tu abilitySetSwapped
local dangDoiBo = false    -- da ban Swap, dang cho server bao ve
local swapLuc   = 0

do
    local r = remote("abilitySetSwapped")
    if r then
        r.OnClientEvent:Connect(function(so)
            if tonumber(so) then boHienTai = tonumber(so) end
            dangDoiBo = false
        end)
    end
end
...
local function banSwap()
    ...
    if not r then return false end
    if dangDoiBo and os.clock() - swapLuc < 3 then return false end  -- dang doi
    if os.clock() - lanSwap < 0.55 then return false end             -- debounce game
    if not pcall(function() r:FireServer() end) then return false end
    lanSwap, swapLuc, dangDoiBo = os.clock(), os.clock(), true
    return true
end

-- F va nut Swap la CUNG mot hanh dong.
local banF = banSwap

-- Dang cho doi bo xong thi chua duoc ban skill (tool chua thay).
dangChoDoiBo = function()
    if not dangDoiBo then return false end
    if os.clock() - swapLuc > 3 then      -- server khong bao ve: thoi khong cho nua
        dangDoiBo = false
        return false
    end
    return true
...
local function ghiSuKien(loai, args)
    ...
    -- Ghi kem SO BO dang dung. Nho no ma luc chay lai biet skill nay thuoc bo
    -- nao, lech nhip Swap van tu chinh lai duoc.
    local bo = nil
    if loai == "q" or loai == "e" then bo = boHienTai end
    macro.sukien[#macro.sukien + 1] =
        { t = bayGio, loai = loai, args = args, bo = bo }
end
    ...
    task.spawn(function()
        ...
        local function thuBanNut(h, tMacro)
            while iSuKien <= #macro.sukien and macro.sukien[iSuKien].t <= tMacro do
                ...
                -- con lai cua macro.
                if s.bo and s.bo ~= boHienTai and (s.loai == "q" or s.loai == "e") then
                    banSwap()
                    return false
                end
                if s.loai == "danh" then
